...
class Heap:
    ...
    def heapifyDown(self, k, n):
        while 2*k + 1 < n:
            ...
            maxI = left if k < n and self.heap[left] > self.heap[k] else k
            if right < n and self.heap[right] > self.heap[maxI]:
                maxI = right
            if maxI != k:
                self.heap[k], self.heap[maxI] = self.heap[maxI], self.heap[k]
                k = maxI
            else:
                break
